...
@section('content')
    <div class="max-w-3xl mx-auto p-6">
        <div class="bg-white rounded-2xl p-6 shadow-sm">
            ...
            <form action="{{ route('master.employees.update', $user->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="contract_start_date"
                            ...
                            class="mt-1 block w-full rounded-md border-gray-200 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" />
                        <p class="text-xs text-gray-500 mt-1">HR/admin can set or adjust the contract start to restart the
                            leave cycle.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="contract_end_date"
                            value="{{ old('contract_end_date', optional($user->contract_end_date)->format('Y-m-d')) }}"
                            class="mt-1 block w-full rounded-md border-gray-200 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" />
                    ...
                    <div>
                        ...
                        <select name="approver_roles[]" multiple
                            ...
                            @foreach (['supervisor', 'manager', 'hod'] as $r)
                                <option value="{{ $r }}"
                                    {{ in_array($r, (array) ($user->approver_roles ?? [])) ? 'selected' : '' }}>
                                    {{ ucfirst($r) }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="primary_supervisor_id" class="mt-1 block w-full rounded-md border-gray-200 shadow-sm">
                            ...
                            @foreach ($supervisors as $sup)
                                <option value="{{ $sup->id }}"
                                    {{ old('primary_supervisor_id', $user->primary_supervisor_id) == $sup->id ? 'selected' : '' }}>
                                    {{ $sup->name }} ({{ $sup->email }})</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="primary_manager_id" class="mt-1 block w-full rounded-md border-gray-200 shadow-sm">
                            ...
                            @foreach ($managers as $mgr)
                                <option value="{{ $mgr->id }}"
                                    {{ old('primary_manager_id', $user->primary_manager_id) == $mgr->id ? 'selected' : '' }}>
                                    {{ $mgr->name }} ({{ $mgr->email }})</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="pt-4">
                        <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded">Update Employee</button>
